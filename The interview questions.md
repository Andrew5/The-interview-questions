...
###### 22、facebook的kvoController的原理，阿里的
答：1. Swinject设计原理
核心概念：
Container类：Swinject的核心是Container类，它提供了register和resolve两类方法。
register方法用于注册服务，将服务的类型、名称和参数类型作为键，存储到内部的services字典中。
resolve方法用于解析服务，根据提供的服务类型和名称，从services字典中查找对应的ServiceEntryProtocol，并调用其方法生成组件实例。
工作原理

依赖注入：Swinject通过Container类实现依赖注入。开发者可以在register时定义服务的依赖关系，resolve时Swinject会自动解析这些依赖，并将它们注入到目标对象中。
灵活性：Swinject支持多种注册方式，包括工厂方法、闭包等，允许开发者根据需要灵活定义服务的创建逻辑。
2. Dagger设计原理

核心概念

注解驱动：Dagger是一个基于注解的依赖注入框架。它通过注解处理器在编译时生成代码，简化了依赖注入的实现。
组件和模块：Dagger的核心是组件（Component）和模块（Module）。模块定义了如何提供依赖项，组件则负责将这些依赖项注入到目标对象中。
工作原理

注解处理器：Dagger的注解处理器会在编译时扫描代码，查找带有特定注解的类和方法。根据这些注解，生成相应的代码，包括组件类和注入器类。
依赖解析：在运行时，Dagger通过生成的组件类来解析依赖关系。组件类会调用模块中定义的@Provides方法来获取依赖项，并将它们注入到目标对象中。
单例支持：Dagger支持单例模式。通过在模块的@Provides方法上添加@Singleton注解，Dagger会确保每次解析该依赖项时返回同一个实例。
3. XEngine设计原理

核心概念

分层存储架构：X-Engine是一个高性能的存储引擎，采用了分层存储架构。它将数据划分为多个层次，根据数据的访问频度将热数据存储在内存中，冷数据存储在持久化存储设备中。
LSM-Tree：X-Engine基于LSM-Tree（Log-Structured Merge Tree）设计，优化了写入性能和空间利用率。
工作原理

内存表和持久化层：热数据和更新操作使用内存表存储，通过无锁跳表（Lock-Free SkipList）提高并发读写性能。访问频度低的数据逐渐淘汰或合并到持久化存储层。
Compaction优化：X-Engine对Compaction过程进行了大量优化，包括拆分数据存储粒度、精细化控制LSM形状，减少I/O和计算代价。
缓存机制：X-Engine使用Row Cache和Block Cache优化读操作，减少缓存失效带来的性能抖动。
4. KVOController原理

核心概念

键值观察（KVO）：KVOController是苹果提供的一个工具，用于简化键值观察（KVO）的使用。它允许开发者以更简洁的方式添加和移除观察者。
自动管理：KVOController自动管理观察者的生命周期，确保观察者在合适的时候被添加和移除。
工作原理

观察者管理：KVOController内部维护了一个观察者列表，当目标对象的属性发生变化时，KVOController会自动通知所有注册的观察者。
线程安全：KVOController保证了线程安全，允许在多线程环境下安全地使用KVO。
简化代码：通过KVOController，开发者可以使用更简洁的代码来实现KVO，减少了手动管理观察者的复杂性。
5. MGJRouter原理

核心概念

路由机制：MGJRouter是一个轻量级的路由框架，用于在iOS应用中实现模块间的跳转。它通过字符串映射的方式，将URL映射到具体的模块或视图控制器。
模块解耦：MGJRouter允许开发者在模块之间通过URL进行通信，而不需要直接引用模块的类。
工作原理

注册路由：开发者需要在应用启动时注册路由，将URL模式映射到具体的处理逻辑。
跳转处理：当需要跳转到某个模块时，MGJRouter会根据注册的路由信息，解析URL并调用对应的处理逻辑。
参数传递：MGJRouter支持通过URL传递参数，允许在模块之间传递数据。
总结

Swinject：通过Container类实现依赖注入，支持灵活的注册方式和自动依赖解析。
Dagger：基于注解的依赖注入框架，通过注解处理器生成代码，支持单例模式。
XEngine：高性能存储引擎，采用分层存储架构和LSM-Tree优化性能。
KVOController：简化KVO的使用，自动管理观察者生命周期。
MGJRouter：轻量级路由框架，通过URL映射实现模块间跳转。

###### 23、组件化如何管理和派发每个模块自身的生命周期
答：1、通过在AppDelegate中手动调用各个模块的代理方法，将生命周期事件分发给各个模块。这种方法简单直接，但需要在AppDelegate中维护模块列表。
   2、通过消息转发机制，将AppDelegate的生命周期方法转发给各个模块。这种方法可以避免在AppDelegate中手动调用模块方法。
   3、通过ModuleManager加载模块，并在AppDelegate的生命周期方法中调用模块的对应方法。这种方法可以将模块的生命周期管理与AppDelegate解耦。
   4、通过XENativeContext类来管理和初始化各个模块（XENativeModule），并确保模块的生命周期独立于壳工程的AppDelegate
        解耦：通过XENativeContext集中管理模块的生命周期，模块之间不需要直接依赖AppDelegate，降低了模块与壳工程之间的耦合度。
        灵活性：模块的初始化顺序可以通过order属性灵活控制，支持动态加载和初始化模块。
        扩展性：新增模块时，只需注册模块类并实现XENativeModule协议，无需修改壳工程代码，便于扩展。
        动态性：支持通过协议动态查询模块实例，提高了组件之间的动态交互能力。
